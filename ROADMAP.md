...
1. **吃透三端 Apple Widget 能力**（护城河）：专注 iOS、iPadOS、macOS 的 Control Center 控件、交互式组件与跨设备同步，把三端的一致创作体验做深。
...
- [~] 新建 `ScriptWidgetRuntimeTests` XCTest target：测试用例已写好（`Tests/ScriptWidgetRuntimeTests/RuntimeUnitTests.swift`，覆盖 prop 类型解析、`displayMessage`、转译缓存键、异常描述）。仅剩在 Xcode 内创建测试 target 并加入此文件这一步（文件头有说明）。
...
### 阶段四 · 三端平台深化
- [ ] 深化 iOS、iPadOS、macOS 的多尺寸预览、交互组件和发布质量。
- [x] AI 辅助脚本生成原型。

> 当前产品边界不包含 watchOS 与 visionOS；研发资源集中在 iOS、iPadOS 和 macOS。
